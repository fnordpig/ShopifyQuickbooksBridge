...
description: >
  ...
  All accept optional date ranges and always produce HTML artifact output.
...
Generate and present data reports with rich markdown tables and HTML artifact
...
Run the report generator. Always generate both markdown and HTML output:
...
python ${CLAUDE_PLUGIN_ROOT}/scripts/generate_report.py \
  ...
  --html-output /tmp/shopify-qbo-report.html
open /tmp/shopify-qbo-report.html
```

The `--html-output` flag is **required** — always pass it so the bookkeeper gets
a formatted, printable view automatically. The HTML file opens in the browser
alongside the markdown summary in chat.
...
## Step 5: Confirm HTML Output

After presenting the markdown report, inform the user that the HTML version
has been opened:

> The full HTML report has been opened in your browser (`/tmp/shopify-qbo-report.html`).
> It includes a formatted, printable view with the same data.

The HTML report uses the generate_report.py script's built-in HTML output.
It follows the project style guide:
- Page background: `#f8f9fa`
- Cards: white with subtle shadow
- Match/OK indicators: `#28a745` (green)
- Mismatch/Error indicators: `#dc3545` (red)
- Warning indicators: `#ffc107` (yellow)
- Font: `system-ui, -apple-system, sans-serif`
- Title includes "Shopify ↔ QBO" branding

If the script's HTML output does not match the style guide, Claude should generate
a corrected HTML file inline and write it to the same path before opening.
