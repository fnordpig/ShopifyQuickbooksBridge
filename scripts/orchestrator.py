...
def run_transform_invoices(
    input_path: str, output_path: str, tax_map_path: str, status_filter: str = "paid"
) -> dict:
...
def validate_sync(customer_output: str, invoice_output: str) -> dict:
    ...
    total_tax_from_invoices = sum(
        inv.get("TxnTaxDetail", {}).get("TotalTax", 0) for inv in invoices
    )
...
def main():
    ...
    parser.add_argument(
        "--shopify-customers", required=True, help="Shopify customers JSON"
    )
    ...
    parser.add_argument(
        "--output-dir", default="./sync_output", help="Output directory"
    )
    parser.add_argument(
        "--status-filter", default="paid", help="Order financial status filter"
    )
    parser.add_argument(
        "--mode",
        default="full",
        choices=["full", "transform-only"],
    ...
    invoice_stats = run_transform_invoices(
        args.shopify_orders, invoice_output, args.tax_map, args.status_filter
    )
    ...
    report_path = generate_audit_report(
        customer_stats, invoice_stats, validation, args.output_dir
    )
    ...
    print(
        f"  Validation:             {'PASS' if validation['valid'] else 'ISSUES FOUND'}"
    )
    print("\n  Output files:")
    ...
    if args.mode == "transform-only":
        ...
        print(
            "  Next step: Agent uses QBO MCP to upsert customers, then create invoices"
        )
