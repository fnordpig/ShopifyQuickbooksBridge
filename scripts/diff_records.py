...
def _match_qbo_customer_to_shopify(qbo: dict, shopify_by_email: dict[str, dict]) -> dict | None:
...
def _diff_customer_fields(expected: dict, actual: dict) -> list[dict]:
    ...
    for field, label in fields_to_compare:
        ...
        if str(exp_val) != str(act_val):
            diffs.append({"field": label, "expected": str(exp_val), "actual": str(act_val)})
...
def _diff_invoice_lines(expected: dict, actual: dict) -> list[dict]:
    ...
    if expected.get("DocNumber") != actual.get("DocNumber"):
        diffs.append({"field": "DocNumber", "expected": expected.get("DocNumber", ""), "actual": actual.get("DocNumber", "")})
    ...
    for i, (el, al) in enumerate(zip(exp_sales, act_sales)):
        ...
        if abs(float(exp_detail.get("UnitPrice", 0)) - float(act_detail.get("UnitPrice", 0))) > 0.01:
            diffs.append({
...
def diff_invoices(shopify_orders: list[dict], qbo_invoices: list[dict], tax_map: dict) -> dict:
...
def main():
    ...
    parser.add_argument("--type", required=True, choices=["customer", "invoice"],
    ...
    parser.add_argument("--tax-map", default=None, help="Path to tax-mapping.json (required for invoice)")
