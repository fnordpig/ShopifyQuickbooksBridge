...
def transform_address(shopify_addr: dict | None) -> dict | None:
    ...
    return {
        ...
        "CountrySubDivisionCode": shopify_addr.get("provinceCode")
        or shopify_addr.get("province", ""),
...
def transform_customer(shopify_customer: dict) -> dict:
    ...
    if not display_name:
        display_name = (
            email or f"Shopify Customer {shopify_customer.get('id', 'unknown')}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Transform Shopify customers to QBO format"
    )
    parser.add_argument(
        "--input", "-i", required=True, help="Path to Shopify customers JSON"
    )
    parser.add_argument(
        "--output", "-o", required=True, help="Path to write QBO customers JSON"
    )
    parser.add_argument(
    ...
    print(
        f"Transformed {stats['total_input']} Shopify customers → {stats['total_output']} QBO customers"
    )
