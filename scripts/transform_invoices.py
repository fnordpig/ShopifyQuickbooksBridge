...
def transform_order(order: dict, tax_map: dict) -> dict:
    ...
    txn_date = created_at[:10] if created_at else datetime.now(timezone.utc).strftime("%Y-%m-%d")
    ...
    # Discount
    total_discounts = to_decimal(order.get("totalDiscounts") or order.get("totalDiscountsSet", {}).get("shopMoney", {}).get("amount", "0"))
    ...
    qbo_invoice["_validation"] = {
        "shopify_total_price": str(order.get("totalPrice") or order.get("totalPriceSet", {}).get("shopMoney", {}).get("amount", "0")),
        "shopify_total_tax": str(order.get("totalTax") or order.get("totalTaxSet", {}).get("shopMoney", {}).get("amount", "0")),
        "shopify_total_discounts": str(total_discounts),
        "shopify_subtotal": str(order.get("subtotalPrice") or order.get("subtotalPriceSet", {}).get("shopMoney", {}).get("amount", "0")),
        "shopify_financial_status": order.get("financialStatus") or order.get("displayFinancialStatus", ""),
        "shopify_fulfillment_status": order.get("fulfillmentStatus") or order.get("displayFulfillmentStatus", ""),
    }
...
def main():
    parser = argparse.ArgumentParser(description="Transform Shopify orders to QBO invoices")
    parser.add_argument("--input", "-i", required=True, help="Path to Shopify orders JSON")
    parser.add_argument("--output", "-o", required=True, help="Path to write QBO invoices JSON")
    parser.add_argument("--tax-map", "-t", required=True, help="Path to tax-mapping.json")
    parser.add_argument("--status-filter", default="paid", help="Shopify financial status filter (default: paid)")
    ...
    if args.status_filter != "all":
        ...
        shopify_orders = [
            o for o in shopify_orders
            if (o.get("financialStatus") or o.get("displayFinancialStatus", "")).lower() == args.status_filter.lower()
    ...
    total_tax = sum(
        Decimal(str(inv["TxnTaxDetail"]["TotalTax"]))
    ...
    print(f"Transformed {stats['total_input']} Shopify orders → {stats['total_output']} QBO invoices")
